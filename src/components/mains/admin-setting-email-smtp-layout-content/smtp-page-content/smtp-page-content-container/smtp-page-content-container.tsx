...
import {
  ...
  sendTestEmailAction,
  updateSmtpAction,
} from '@/actions/smtp-action';
import { IUserResponse } from '@/interfaces/user-interface';
import { ActionResponse } from '@/interfaces/_base-interface';

interface SmtpPageContentContainerProps {
  smtp: ISmtpResponse | null;
  userData: IUserResponse;
}

export default function SmtpPageContentContainer({
  smtp,
}: SmtpPageContentContainerProps) {
  const [host, setHost] = useState(smtp?.host || '');
  const [port, setPort] = useState<number | string>(smtp?.port || 587);
  ...
  const [receiveEmail, setReceiveEmail] = useState(smtp?.receiveEmail || '');
  ...
  const handleSaveSmtp = async () => {
    ...
    try {
      let response: ActionResponse<ISmtpResponse>;

      if (smtp?.id) {
        const updatePayload: IUpdateSmtp = {
          host,
          port: Number(port),
          username,
          secure,
        ...
        response = await updateSmtpAction(smtp.id, updatePayload);
      } else {
        if (!password) {
        ...
        const createPayload: ICreateSmtp = {
          host,
          port: Number(port),
          username,
          password,
        ...
        response = await saveSmtpAction(createPayload);
      }
      ...
      if (response.success) {
        ...
        notifications.show({
          message: response.error || 'Failed to save configuration',
          color: 'red',
          position: 'top-right',
        });
      }
      ...
      <Paper radius={'md'} shadow="xs" classNames={{ root: classes.paperBlock }}>
        <Stack p={'sm'} gap={'md'}>
          <Group justify="space-between">
            ...
            <Button onClick={handleSaveSmtp}>Save Changes</Button>
          </Group>
            ...
            <Group justify="space-between" align="flex-end">
              <Stack gap={2} style={{ flex: 1 }}>
                ...
                <TextInput
                  ...
                  value={receiveEmail}
                  onChange={(e) => setReceiveEmail(e.currentTarget.value)}
                />
              </Stack>
              <Button
